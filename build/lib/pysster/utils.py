...
def save_model(model, file_path):
    ...
    model.model.save(f"{file_path}.keras")


def load_model(file_path,data):
    ...
    if not  os.path.exists(f"{file_path}.keras"):
        raise RuntimeError("Keras file not found.")
    with gzip.open(file_path, "rb") as handle:
        params = pickle.load(handle)
    keras_model = keras.models.load_model(f"{file_path}.keras")
